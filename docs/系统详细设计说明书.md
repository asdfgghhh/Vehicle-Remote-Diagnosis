...
| ② | 网关层 | service-gateway | 统一 API 入口，StripPrefix 路由，JWT 内省前置校验 |
| ③ | 服务层 | 7 个业务微服务 | auth/vehicle/ecu-log/dbc/signal/access/diagnosis |
| ④ | 数据层 | MySQL / Redis / ClickHouse / 对象存储 | 事务数据、缓存、时序分析、文件实体 |
| ⑤ | 中间件 | Kafka / EMQX | 异步消息与物联网接入 |
...
- service-vehicle → MySQL（vrd_vehicle）、Kafka（vehicle-data/vehicle-signals）
...
- service-diagnosis → MySQL（vrd_diagnosis）、Kafka（uds-commands）
...
1. **信号链路**：车端 → MQTT/HTTP → service-access → Kafka `vehicle-signals` → ClickHouse → service-signal 查询（service-vehicle 同步消费做告警规则评估）
...
**诊断指令（下行）**：管理端 → service-gateway → service-diagnosis → Kafka `uds-commands` → 车端
...
- **基础设施层**：MySQL、Redis、Kafka、EMQX、Nacos、ClickHouse（部署于宿主机 124.221.104.56，不在编排内）
- **微服务层**：gateway、auth、vehicle、ecu-log、dbc、signal、access、diagnosis（均注册至 Nacos）
...
**路由规则**（`backend/nacos-configs/service-gateway.yml`，经 Nacos 下发）：
...
| `/api/diagnosis/**` | service-diagnosis | 1 |

**认证过滤器 `AuthFilter`**（`com.vrd.gateway.config.AuthFilter`）：

- 白名单：`/api/auth/login`、`/api/auth/register`、`/api/auth/introspect`、`/actuator/**`
- 其他请求：检查 `Authorization: Bearer ` 前缀，缺失则返回 HTTP 401
- 通过 WebClient 调用 service-auth `/auth/introspect` 校验 Token 有效性，校验通过后将 `X-User-Id`、`X-Username`、`X-Roles` 注入下游请求头
...
**预置角色**（`backend/sql/mysql/02_vrd_auth.sql`）：
...
| alert_rule | 告警规则（阈值/趋势/组合，Easy Rules） |
| alert_trigger_log | 告警触发日志 |
...
**DBC 解析**：纯 Java 原生实现（`parser/DbcParser` + `parser/CanFrameCodec`），完整支持 BO_、SG_、CM_、VAL_、BA_、SG_MUL_VAL_ 规范，精确实现 Intel/Motorola 两种字节序位运算，零外部依赖（替代早期 Python cantools 方案）。

**核心流程**：

1. `POST /dbc/upload`：接收 multipart 文件 → 原生解析 CAN 消息/信号 → 写入 MySQL `dbc_file`（含 parse_result、message_count、signal_count）
2. `GET /dbc/{id}/messages`：从 parse_result 提取消息名列表
3. `GET /dbc/{id}/signals`：提取信号定义（name、startBit、length、factor 等）
4. `GET /dbc/{id}/structured`：结构化解析结果查询；`/message/{messageId}` 报文详情；`/signal/{signalName}` 信号详情
5. `POST /dbc/{id}/decode`：CAN 帧实时解码（Intel/Motorola 位运算）
6. `POST /dbc/{id}/encode`：CAN 帧编码测试
7. `GET /dbc/{id}/java-constants`、`/json-schema`：代码生成
8. `POST /dbc/{id}/dispatch/{vehicleId}` 或 `/dispatch`：下发到车端，记录 `dispatch_log`
...
| `db` 子包 | MyBatis-Plus 分页插件、逻辑删除全局字段 `deleted` |
...
| /config/can-model | CanModel.vue | DBC/CAN 模型配置 |
| /config/can-model/:id | CanModelDetail.vue | CAN 模型详情 |
| /config/fault | FaultConfig.vue | 故障配置 |
...
**未接入路由的页面**（组件已就绪，待接入）：`SignalMonitor.vue`（实时信号监控）、`UdsDiagnosis.vue`（UDS 远程诊断）、`AlertRules.vue`（告警规则）、`AlertLogs.vue`（告警记录）。

**路由守卫**：仅检查 localStorage 中 token 是否存在，不校验角色权限。

**开发代理**：Vite 将 `/api` 代理至 `http://localhost:9080`。

---

### 3.10 service-diagnosis（UDS 远程诊断服务）

**职责**：基于 ISO 14229 的 UDS 诊断协议服务，管理端发起诊断请求，经 Kafka 下发车端，并记录诊断会话历史。

**核心类**：

| 类 | 说明 |
|----|------|
| `UdsDiagnosisController` | REST 控制器：14 个诊断端点 + 服务列表 + 会话历史 |
| `UdsDiagnosisService` / `Impl` | UDS 核心实现：会话控制、ECU 复位、安全访问、DID 读写、DTC 读/清除、例程控制、内存读写、IO 控制、Tester Present |
| `UdsCommandProducer` | 诊断指令 Kafka 生产者（topic: `uds-commands`） |
| `UdsServiceId` | SID 常量（0x10-0x3E）、子功能码、NRC 否定响应码 |
| `UdsRequest` / `UdsResponse` | 请求/响应 DTO（vin, serviceId, subFunction, traceId, nrc 等） |
| `UdsDiagnosisSessionMapper` / `UdsDtcRecordMapper` | MyBatis-Plus Mapper |

**数据模型**（数据库 `vrd_diagnosis`）：

- `uds_diagnosis_session`：诊断会话（traceId, vin, serviceId, requestData, responseData, requestTime 等）
- `uds_dtc_record`：DTC 故障码（dtcCode, dtcStatus, severity, faultStatus）

**数据流**：管理端 → `/api/diagnosis/**`（网关）→ service-diagnosis → 构建 UDS 指令 → Kafka `uds-commands` → 车端；会话记录写入 MySQL。
...
| vrd_vehicle | service-vehicle | vehicle_model, vehicle, vehicle_ecu, vehicle_alert, vehicle_fault, sync_log, alert_rule, alert_trigger_log 等 |
...
| vrd_diagnosis | service-diagnosis | uds_diagnosis_session, uds_dtc_record |
...
| 认证 | JWT + BCrypt 密码哈希，网关内省校验 | 短 Token 有效期 + 刷新机制 |
...
| service-auth | 9081 |
| service-vehicle | 9082 |
| service-ecu-log | 9083 |
| service-dbc | 9084 |
| service-signal | 9085 |
| service-access | 9086 |
| service-diagnosis | 9087 |
| frontend | 3000 |
| Nacos | 8848 |
| Kafka | 9092 |
| MQTT | 1883 |
...
| uds-responses 消费 | 未实现 | 诊断响应回传主题已规划，车端回执消费未实现 |
| Eureka service-register | 遗留 | 代码存在，未编入 backend/pom.xml modules |
| 前端优化页面接入 | 未接入 | SignalMonitor/UdsDiagnosis/AlertRules/AlertLogs 组件已就绪，待接入主路由菜单 |
