...
| 编写依据 | 项目源码、`backend/sql/mysql/` 初始化脚本、`docker-compose.yml` |
...
| 服务 | 端口 | 职责 |
|------|------|------|
| service-gateway | 9080 | API 统一入口、路由转发、CORS、JWT 内省校验 |
| service-auth | 9081 | 登录注册、JWT 签发/内省、用户/角色管理 |
| service-vehicle | 9082 | 车型/车辆/ECU、仪表盘统计、Kafka/API 同步、告警规则引擎 |
| service-ecu-log | 9083 | ECU 日志查询与下载（读 ClickHouse） |
| service-dbc | 9084 | DBC 文件上传解析（纯 Java 原生解析器）、下发管理 |
| service-signal | 9085 | 车辆信号时序查询（读 ClickHouse） |
| service-access | 9086 | 车端统一接入：MQTT/HTTP 信号、日志上传、WebSocket 推送 |
| service-diagnosis | 9087 | UDS 远程诊断（ISO 14229）、诊断指令 Kafka 下发、会话历史 |

**公共模块**：`common`（统一响应、存储抽象、Nacos 配置、MyBatis-Plus 分页配置，`com.vrd.common.db` 子包）。
...
| UDS 诊断 | 会话控制、DID/DTC 读写、安全访问、指令下发 | service-diagnosis |
| 告警监控 | 阈值/趋势/组合规则、告警触发日志 | service-vehicle |
...
```
浏览器 → frontend(:3000) → gateway(:9080/api/*) → 各微服务 → MySQL/ClickHouse/对象存储
```
...
| vehicle-signals | service-access | service-access / service-vehicle | 信号入库 ClickHouse、告警规则评估 |
| uds-commands | service-diagnosis | 车端（预留） | UDS 诊断指令下发 |
| uds-responses | 车端（预留） | service-diagnosis（预留） | UDS 诊断响应回传 |
...
默认管理员：`admin`（密码见 `backend/sql/mysql/02_vrd_auth.sql` 预置 BCrypt 哈希）。
...
1. 网关调用 service-auth `/auth/introspect` 内省校验 JWT；RBAC 未在 API 层强制。
2. `service-bigdata`、Doris 分析链路尚未实现；UDS 诊断指令已通过 Kafka 下发，车端回执链路（uds-responses 消费）预留。
3. 车端接入接口（`/ecu-log/vehicle/*`、`/signal/vehicle/receive`）未列入网关白名单，需携带 Token 或后续单独开放。
4. 前端优化页面（实时信号监控、UDS 诊断、告警规则/记录）组件已就绪，尚未全部接入主路由菜单。
